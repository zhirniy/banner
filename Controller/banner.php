...
if(empty($visit_id)){
    $banner->delete('ips');
    $banner->insert_ips('ip_address', $visitor_ip);
    $banner->insert_visits('date', $date, 'hosts=1', 'views=1');
}
...
if ($_GET['days']=='all'){
    $result_week_views = $banner->count('views');
    $result_day_views = $result_week_views;
    $result_week_hosts = $banner->count('hosts');
...
$result_all = $banner::property_visits('*',1,1);
$result_week_views = $banner->count('views');
$result_day_views = $result_week_views;
$result_week_hosts = $banner->count('hosts');
$result_day_hosts = $result_week_hosts;}
...
if($_GET['days']=='ip_all'){
$result_ips = $banner::property_ips('*',1,1);
}
